...
async fn main() -> Result<()> {
    ...
    let tokenIn: Address = address!("B31f66AA3C1e785363F0875A1B74E27b85FD66c7"); // WAVAX on Avalanche
    let tokenOut: Address = address!("B97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E"); // USDC on Avalanche
    let quoterAddress: Address = address!("be0F5544EC67e9B3b2D979aaA43f18Fd87E6257F"); // Quoter on Avalanche
    // let calldata = quote_calldata(WETH_ADDR, USDC_ADDR, volume, 3000);
    let calldata = quote_calldata(tokenIn, tokenOut, volume, 3000);

    // let tx = build_tx(quoterAddress, ME, calldata, base_fee);
    let chain_id = 43114;
    let tx = build_tx_avalanche(quoterAddress, ME, calldata, base_fee, Some(chain_id));
